<?php
/**
 * services/category/edit.php
 * Cập nhật tên danh mục
 *
 * Schema: danhmuc(idDM, LOAISP)
 * — Không có MOTA, không có TRANGTHAI trong schema hiện tại
 *
...
try {
    ...
    // Cập nhật — schema chỉ có LOAISP, không có MOTA / TRANGTHAI
    $stmt = $conn->prepare("UPDATE danhmuc SET LOAISP = ? WHERE idDM = ?");
    $stmt->bind_param('si', $name, $id);
    ...
    @file_put_contents(__DIR__ . '/../../logs/index/dashboard.text',
        date('[Y-m-d H:i:s]') . ' [CATEGORIES/edit] ' . $e->getMessage() . "\n",
        FILE_APPEND
    );
}
